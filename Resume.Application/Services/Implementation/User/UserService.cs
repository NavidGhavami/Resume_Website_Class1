...
public class UserService : IUserService
{
    ...
    public async Task<EditUserResult> EditUser(EditUserDto command, IFormFile avatarImage)
    {
        var user = await _userRepository
            .GetQuery()
            .AsQueryable()
    ...
    public async Task<LoginUserResult> LoginUser(LoginUserDto userLogin)
    {
        var hashPassword = _passwordHasher.EncodePasswordMd5(userLogin.Password);

        var user = await _userRepository
            .GetQuery()
            .AsQueryable()
            .FirstOrDefaultAsync(x => x.Mobile == userLogin.Mobile && x.Password == hashPassword);
        ...
        if (user.Mobile != userLogin.Mobile && user.Password != hashPassword)
        {
            return LoginUserResult.NotUserFound;
        }
